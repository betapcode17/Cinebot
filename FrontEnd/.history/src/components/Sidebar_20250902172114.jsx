...
const Sidebar = () => {
  ...
  const [timeWindow, setTimeWindow] = useState("day"); // "day" hoặc "week"
  ...
  useEffect(() => {
    const fetchTrending = async () => {
      try {
        const res = await getTrendingMovies(timeWindow); // Truyền timeWindow nếu API hỗ trợ
  ...
  useEffect(() => {
    const fetchComingSoon = async () => {
      try {
        const res = await getTrendingMovies("week"); // Có thể thay bằng endpoint khác
        const items = res.data?.data?.items || res.data?.results || [];
        const soonItems = items
          ...
          .slice(0, 5);
        setComingSoon(soonItems.length > 0 ? soonItems : items.slice(0, 5));
      } catch (err) {
