...
import { getSingleMovies, getSeriesMovies } from "../api/movieApi"; // Import các hàm API
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        let data;
        if (type === "tv") {
          data = await getSeriesMovies(1, 20); // Lấy 20 phim bộ
        } else {
          data = await getSingleMovies(1, 20); // Lấy 20 phim lẻ
        }
    ...
    <div className="mt-10 bg-[#0f0f0f] border border-[#333] rounded-md p-4 shadow-md">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
        {items.length > 0 ? (
          items.map((item) => (
            <Link
              to={`/${type === "tv" ? "tv" : "movie"}/${item.slug || item._id}`} // Sử dụng id hoặc _id
