  ...
  useEffect(() => {
    const fetchMovies = async () => {
      ...
      try {
        ...
        setMovies(res.data.data.items || []); // Adjust based on actual API response structure
        setTotalPages(res.data.data.pagination?.totalPages || 1); // Adjust based on actual API response
      } catch (err) {
    ...
    <div className="bg-[#111] px-4 py-6 text-white">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
          {movies.map((movie) => (
            <MovieCard key={movie.slug} movie={movie} type="movie" /> // Changed `data` to `movie`
          ))}
        </div>
