...
const CinemaMovies = () => {
  ...
  const fetchMovies = async () => {
    setIsLoading(true);
    try {
      const { items, totalPages } = await getSingleMovies(page, limit);
      setMovies(items || []);
      // Giới hạn tối đa 10 page như yêu cầu
      setTotalPages(Math.min(totalPages, 10));
    } catch (err) {
      console.error(
  ...
  // Gọi lại khi page hoặc filters thay đổi
  useEffect(() => {
    fetchMovies();
  }, [page, filters]);
