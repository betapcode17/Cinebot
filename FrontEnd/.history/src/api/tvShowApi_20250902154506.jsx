...
export const getSeriesMovies = async (filters = {}, page = 1, limit = 10) => {
  try {
    ...
    if (filters.genre) {
      params.category = filters.genre; // Adjust based on API documentation
    }
    if (filters.country) {
      params.country = filters.country; // Adjust based on API documentation
    }
    if (filters.year) {
      params.year = filters.year; // Adjust based on API documentation
    }
    if (filters.sortBy) {
      params.sort_field = filters.sortBy; // Adjust based on API documentation
    }
